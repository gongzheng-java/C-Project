...
namespace DataImport
{
    class DbHelper
    {
        ...
        //数据库连接字符串
        //private static string connectionString =string.Format("Database='{0}';Data Source='{1}';User Id='{2}';Password='{3}';charset='utf8';pooling=true;port=3306",m_Server,m_Database,m_User,m_Password);
        ...
        {
            //数据库连接字符串
            string connectionString = string.Format("Database='{0}';Data Source='{1}';User Id='{2}';Password='{3}';charset='utf8';pooling=true;port=3306", m_Database, m_Server, m_User, m_Password);
        ...
        public static int Count(CommandType cmdType, string cmdText, params MySqlParameter[] commandParameters)
        {
        ...
        public static void BindToCombox(ComboBox box, string sql)
        {
            DataSet ds = GetDataSet(CommandType.Text, sql, null);
            if (ds == null)
                return;

            box.Items.Clear();
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                box.Items.Add(ds.Tables[0].Rows[i].ItemArray[0]);
